    ...
    public class AuditHistory
    {
        ...
        public static string CreateAuditLogTableSql
        {
            get
            {
                ...
                tableSqlBuilder.AppendLine(@$"  [TableName] [nvarchar] (200) NULL,");
                tableSqlBuilder.AppendLine(@$"  [RowId] [bigint] NULL,");
                ...
                tableSqlBuilder.AppendLine(@$"  [CreatedBy] [nvarchar] (100) NULL,");
                tableSqlBuilder.AppendLine(@$"  [CreatedAt] [datetime2](7) NULL CONSTRAINT [PK_CommonAudit{DateTime.Now:yyyyMM}] PRIMARY KEY CLUSTERED ([Id] ASC) WITH (");
            ...
            get
            {
                if (_rows != null && _rows.Any())
                {
                    ...
                    for (int i = 0; i < _rows.Count; i++)
                    {
                        ...
                        if (i == _rows.Count - 1)
                        {
                            sqlDataBuilder.AppendLine(@$"(N'{log.TableName}',{long.Parse(log.RowId ?? "0")},N'{log.Action}',N'{log.Row.Replace("'", "\"\"")}','{DateTime.Now.GetStringTypeTimeStamp()}',N'{log.UpdatedBy}',GETDATE());");
                        }
                        else
                        {
                            sqlDataBuilder.AppendLine(@$"(N'{log.TableName}',{long.Parse(log.RowId ?? "0")},N'{log.Action}',N'{log.Row.Replace("'", "\"\"")}','{DateTime.Now.GetStringTypeTimeStamp()}',N'{log.UpdatedBy}',GETDATE()),");
